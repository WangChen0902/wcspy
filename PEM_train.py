...
if __name__ == "__main__":
    ...
    optimizer,loss=PEM_Train(X_feature,Y_iou,LR,config)
    ...
    val_data=PEM_load_data.getTrainData(config.batch_size,"val")
    ...
    save_file = 'PEM-log-'+nowTime+'.txt'
    f = open(save_file, 'w+')
    f.write('learning_rates: 0.01 \n')
    ...
    ## TRAIN ##
        ...
        for idx in range(len(train_data)):
            ...
            #print(len(batch_iou_list))
            for b in range(len(batch_feature)):
                c_list = [('%.05f' % (a)) for a in batch_feature[b]]
                iou_str = '%.05f\n'% (batch_iou_list[b])
                fw.write(str(c_list))
                fw.write(iou_str)
            fw.close()
            _,out_loss=sess.run([optimizer,loss],feed_dict={X_feature:batch_feature,
        ...
        train_info_str = "Epoch-%d Train Loss:  %.04f" %(epoch,train_info["iou_loss"][-1])
        val_info_str = "Epoch-%d Val   Loss:  %.04f" %(epoch,val_info["iou_loss"][-1])
